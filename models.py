...
class Encoder(nn.Module):
    ...
    def forward(self, X):
        ...
        X, (hidden_state, cell_state) = self.lstm(
            X)
...
class Decoder(nn.Module):
    def __init__(self, source_vectors,
                 target_vocab_size, embedding_dim,
        ...
        self.fc = nn.Linear(hidden_dim * 2,
                            target_vocab_size)  # bidrectional hence
    ...
    def forward(self, hidden_state, cell_state, Y,
        ...
        batch_size, seq_len = Y.shape[0], Y.shape[
            1]
        outputs = torch.zeros(seq_len, batch_size,
                              self.target_vocab_size,
                              requires_grad=True).to(
            device)  # [Batch_Size X Sequence_Len]
        ...
        for i in range(seq_len):
            ...
            assert decoder_input.shape[0] > 0 and \
                   decoder_input.shape[1] > 0
            decoder_output, (
            hidden_state, cell_state) = self.lstm(
                decoder_input,
                (hidden_state, cell_state))
            # Decoder_Output_Shape = [Batch_Size X 1 X Target_Vocab_Size]
            decoder_output = self.fc(
                decoder_output)
            # Store output
            outputs[i] = decoder_output.permute(1,
                                                0,
                                                2).squeeze(
                0)
            ...
            X = indexes if random.random() < 0.5 else Y[
                                                      :,
                                                      i]
...
class MultiHeadAttention(nn.Module):
    ...
    def scaled_dot_product_attention(self, Q,
                                     K, V,
                                     mask=None):
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        attn_output = self.scaled_dot_product_attention(
            Q, K, V, mask)
        output = self.W_o(
            self.combine_heads(attn_output))
        return output
